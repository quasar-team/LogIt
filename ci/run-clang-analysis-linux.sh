...
cmake -S . -B "$BUILD_DIR" \
  ...
  -DLOGIT_BACKEND_STDOUTLOG=ON \
  -DLOGIT_BACKEND_BOOSTLOG=ON \
  -DLOGIT_BACKEND_UATRACE=ON \
  -DLOGIT_BACKEND_WINDOWS_DEBUGGER=OFF

log "Building LogIt once without analyzers"
cmake --build "$BUILD_DIR" --config Release --target LogIt

log "Marking project sources as modified for analyzer-only rebuild"
find src include -type f \( -name '*.c' -o -name '*.cc' -o -name '*.cpp' -o -name '*.cxx' -o -name '*.h' -o -name '*.hpp' -o -name '*.hxx' \) -exec touch {} +
...
scan-build --keep-empty -plist -o "$SCAN_REPORT_DIR" --use-cc=clang --use-c++=clang++ \
  cmake --build "$BUILD_DIR" --config Release --target LogIt |& tee "$SCAN_LOG"
...
for plist_path in report_root.rglob("*.plist"):
    ...
    for d in diagnostics:
        ...
        rel = str(rel_path)
        if not (rel.startswith("src/") or rel.startswith("include/")):
            continue
...
tidy_status=0
rm -f "$TIDY_LOG"
mapfile -t tidy_files < <(find src -type f \( -name '*.c' -o -name '*.cc' -o -name '*.cpp' -o -name '*.cxx' \) ! -name 'WindowsDebuggerSink.cpp' | sort)
...
repo_root = pathlib.Path(sys.argv[2]).resolve()
diagnostic_re = re.compile(r"^(.*?):(\d+):(\d+):\s+(warning|error):\s+(.*)$")
...
if log_path.exists():
    for raw_line in log_path.read_text(errors="replace").splitlines():
        ...
        path_text, line_no, col_no, level, message = match.groups()
        raw_path = pathlib.Path(path_text)
        abs_path = raw_path if raw_path.is_absolute() else (repo_root / raw_path)
        try:
        ...
        diagnostic = f"{rel}:{line_no}:{col_no}: {level}: {message}"
...
print(f"CLANG_TIDY_PROJECT_COUNT={len(hits)}")
PY
)
tidy_project_count="$(echo "$tidy_summary" | awk -F= '/^CLANG_TIDY_PROJECT_COUNT=/{print $2}' | tail -n1)"
tidy_project_count="${tidy_project_count:-0}"

if [[ $tidy_status -ne 0 ]]; then
  log "clang-tidy findings (project sources only):"
  echo "$tidy_summary"
  if [[ $tidy_project_count -eq 0 ]]; then
    log "clang-tidy exited with errors, but no project diagnostics were parsed. Full log: ${TIDY_LOG}"
  fi
fi

if [[ $scan_invocation_failed -ne 0 || $scan_project_count -ne 0 || $tidy_status -ne 0 ]]; then
  log "ERROR: analyzer failures detected (scan-invocation-failed=$scan_invocation_failed, scan-build-exit=$scan_build_status, scan-project-findings=$scan_project_count, clang-tidy=$tidy_status, clang-tidy-project-findings=$tidy_project_count)"
  exit 1
fi
